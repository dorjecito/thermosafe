...
  const body = `${info.event || "Avís meteorològic actiu"}${
    place ? " · " + place : ""
  }`;
...
        try {
          const zoneKey =
            `${sub.lat.toFixed(1)},${sub.lon.toFixed(1)}`;
...
          await sendRiskNotification(sub.token, alerts);

        } catch (err) {
          console.error("cron aemet error", sub.token, err);
        }
      })
    );
