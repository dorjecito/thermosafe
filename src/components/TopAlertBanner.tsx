...
type TopAlertBannerInput = Omit<Props, "weatherMain" | "weatherContext"> & {
  rainy: boolean;
};

export function getTopAlertBannerState({
  primary,
  heatRisk,
  uvi,
  day,
  ...
  UV_HIGH,
  UV_EXTREME,
}: TopAlertBannerInput): TopAlertBannerState {
  if (aemetActive) {
    return {
      severity: 3,
      key: "aemet-active",
      content: `🚨 ${t("top_alert_weather_active") || "Active weather alert"}`,
      pulse: true,
  ...
  if (aemetSoon) {
    return {
      severity: 2,
      key: "aemet-soon",
      content: `⚠️ ${t("official_alert_soon") || "Upcoming official weather alert"}`,
      pulse: false,
...
export default function TopAlertBanner({
  ...
  }, [aemetActive, aemetSoon, primary, heatRisk, uvi, day, rainy, clouds, irr, t, UV_HIGH, UV_EXTREME]);
